...
public class ExternalProject {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (Client client = JMemcachedClientFactory.buildNewClient("localhost", 9010)) {
            ...
            client.put("value_int", 50); //Сохранить текст под ключом test на сервере
            client.put("value_float", 50.555); //Сохранить текст под ключом test на сервере

            list.add(client.get("value_int"));
            list.add(client.get("value_float"));

            System.out.println(list);

            client.put("test", "Hello world"); //Сохранить текст под ключом test на сервере
            System.out.println(client.<String>get("test")); //Запросить объект по ключу test

            client.remove("test"); //Удалить объект по ключу test
            System.out.println(client.<String>get("test")); //Должно отобразить null - объект удален

            client.put("test", "Hello world");
            client.put("test", new BusinessObject("TEST"));
            // Выполнить замену объекта по ключу test
            System.out.println(client.<BusinessObject>get("test")); // Увидим BusinessObject
            ...
            System.out.println((client.<String>get("devstudy"))); //Должно отобразить null - объект удален, истекто время жизни 2 сек
        }
    }
